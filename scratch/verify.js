const regionsData = [
  ...
  // --- 흥덕구 하위 (1읍, 2면, 8동) ---
  { id: "heungdeok-osong", keywordName: "오송읍", formalName: "청주시 흥덕구 오송읍", regionType: "eup", parentId: "heungdeok-osong", nearbyRegionIds: [], keywordVariantType: "formal" },
  { id: "heungdeok-gangnae", keywordName: "강내면", formalName: "청주시 흥덕구 강내면", regionType: "myeon", parentId: "heungdeok-gangnae", nearbyRegionIds: [], keywordVariantType: "formal" },
  { id: "heungdeok-oksan", keywordName: "옥산면", formalName: "청주시 흥덕구 옥산면", regionType: "myeon", parentId: "heungdeok-oksan", nearbyRegionIds: [], keywordVariantType: "formal" },
...
regionsData.forEach(r => {
  if (r.regionType === 'si' && r.keywordVariantType === 'formal') stats.si++;
  else if (r.regionType === 'gun') stats.gun++;
  else if (r.regionType === 'gu') stats.gu++;
  else if (r.regionType === 'eup') stats.eup++;
  else if (r.regionType === 'myeon') stats.myeon++;
  else if (r.regionType === 'dong') stats.dong++;
  
  if (r.keywordVariantType === 'alias') stats.alias++;

  // 중복 공식 명칭 체크
  const rawName = r.formalName.split(' ').pop();
  if (r.regionType === 'dong' || r.regionType === 'eup' || r.regionType === 'myeon') {
    if (names[rawName]) {
      names[rawName].push(r);
    } else {
      names[rawName] = [r];
    }
  }
...
for (const name in names) {
  if (names[name].length > 1) {
    duplicates.push({
      name,
      occurrences: names[name].map(x => `${x.formalName} (${x.id})`)
    });
  }
}
