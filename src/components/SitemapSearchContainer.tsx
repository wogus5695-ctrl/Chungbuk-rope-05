...
export default function SitemapSearchContainer({ regions, services }: SitemapSearchContainerProps) {
  ...
  const mainGroups = [
    ...
    { id: "yeongdong-gun", name: "영동군", province: "chungbuk" },
  ...
  // 카드 렌더링 헬퍼: 소제목에는 displayName 대신 keywordName만 단독으로 표시하여 formalName이나 기술적 표기가 노출되지 않도록 처리
  const renderRegionCard = (reg: DetailedRegion) => {
    const isRegionMatched = searchQuery === "" || 
      reg.formalName.toLowerCase().includes(searchQuery) || 
      reg.keywordName.toLowerCase().includes(searchQuery) ||
      reg.displayName.toLowerCase().includes(searchQuery);
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {services.map(svc => {
            ...
            const linkText = `${reg.displayName} ${svc.keyword}`;
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-xs flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 relative">
          ...
          <input 
            id="region-search"
            type="text" 
            placeholder="동네 이름 검색 (예: 북문로, 남문로, 용암동, 상당구, 대전)"
            value={searchQuery}
      ...
      <div className="space-y-4">
        {mainGroups.map(group => {
          ...
          const totalMatchedCount = topLevelRegions.filter(reg => {
            return searchQuery === "" || reg.formalName.toLowerCase().includes(searchQuery) || reg.keywordName.toLowerCase().includes(searchQuery) || reg.displayName.toLowerCase().includes(searchQuery);
          }).length + (hasSubGus ? regions.filter(r => {
            const isChildOfGu = subGus.some(g => g.id === r.parentId);
            return isChildOfGu && (searchQuery === "" || r.formalName.toLowerCase().includes(searchQuery) || r.keywordName.toLowerCase().includes(searchQuery) || r.displayName.toLowerCase().includes(searchQuery));
          }).length : 0);
